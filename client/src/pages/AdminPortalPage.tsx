...
const sections = [
  ...
  { key: "documents", label: "Class-list files", icon: FileText },
];
...
function DocumentManager() {
  const utils = trpc.useUtils(); const docs = trpc.admin.documents.list.useQuery(); const upload = trpc.admin.documents.upload.useMutation({ onSuccess: async () => { await utils.admin.documents.list.invalidate(); await utils.admin.dashboard.invalidate(); toast.success("Class-list file uploaded."); }, onError: error => toast.error(error.message) });
  return <Card className="rounded-2xl border-0 shadow-sm"><CardHeader><CardTitle className="font-serif text-2xl text-[#153f3b]">Class-list documents</CardTitle><CardDescription>Upload a PDF or document. Files are stored privately and retained as references for the import workflow.</CardDescription></CardHeader><CardContent className="space-y-5"><input type="file" accept=".pdf,.doc,.docx,.csv" onChange={event => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => upload.mutate({ filename: file.name, mimeType: file.type || "application/octet-stream", dataBase64: String(reader.result).split(",")[1] ?? "" }); reader.readAsDataURL(file); }} />{docs.data?.length ? <div className="space-y-2">{docs.data.map(doc => <a key={doc.id} href={doc.storageUrl} target="_blank" rel="noreferrer" className="block rounded-xl bg-[#f8f4ee] p-3 text-sm text-[#153f3b] hover:underline">{doc.filename}</a>)}</div> : <p className="text-sm text-slate-500">No class-list documents uploaded yet.</p>}</CardContent></Card>;
}

function UpdatesManager() {
  const utils = trpc.useUtils(); const updates = trpc.admin.updates.list.useQuery(); const create = trpc.admin.updates.create.useMutation({ onSuccess: async () => { await utils.admin.updates.list.invalidate(); await utils.admin.dashboard.invalidate(); toast.success("Urgent update published."); setTitle(""); setBody(""); }, onError: error => toast.error(error.message) }); const [title, setTitle] = useState(""); const [body, setBody] = useState("");
  return <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]"><Card className="rounded-2xl border-0 shadow-sm"><CardHeader><CardTitle className="font-serif text-2xl text-[#153f3b]">Publish urgent update</CardTitle><CardDescription>These notices are reserved for authenticated parent communication.</CardDescription></CardHeader><CardContent><form className="space-y-4" onSubmit={event => { event.preventDefault(); create.mutate({ title, body, isPublished: true }); }}><Input placeholder="Update title" value={title} onChange={event => setTitle(event.target.value)} required /><Textarea className="min-h-32" placeholder="Write the message for parents" value={body} onChange={event => setBody(event.target.value)} required /><Button className="rounded-xl bg-[#153f3b] text-white hover:bg-[#235b55]" disabled={create.isPending}>Publish update</Button></form></CardContent></Card><Card className="rounded-2xl border-0 shadow-sm"><CardHeader><CardTitle className="text-base">Published and draft updates</CardTitle></CardHeader><CardContent>{updates.data?.length ? updates.data.map(update => <div key={update.id} className="mb-3 rounded-xl bg-[#f8f4ee] p-4"><p className="font-medium text-[#153f3b]">{update.title}</p><p className="mt-1 text-sm text-slate-600">{update.body}</p></div>) : <p className="text-sm text-slate-500">No updates yet.</p>}</CardContent></Card></div>;
}
...
export default function AdminPortalPage() {
  ...
  useEffect(() => {
    ...
    setSection(routeSection === "content" || routeSection === "learners" || routeSection === "parents" || routeSection === "marks" || routeSection === "updates" || routeSection === "documents" ? routeSection : "overview");
  }, [location]);
