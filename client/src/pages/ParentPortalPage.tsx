...
export default function ParentPortalPage() {
  ...
  const utils = trpc.useUtils(); const portal = trpc.parent.portal.useQuery(undefined, { enabled: signedIn }); const markRead = trpc.parent.markUpdateRead.useMutation({ onSuccess: async () => { await utils.parent.portal.invalidate(); } });
        ...
        <section className="px-5 py-10 sm:py-14 lg:py-16">
          <div className="mx-auto max-w-[450px]">
            <div className="rounded-[1.8rem] border border-[#e6dfd7] bg-white p-6 shadow-[0_12px_30px_rgba(62,44,24,.08)] sm:p-7">
              ...
              {signedIn && portal.data && <div className="mt-6 rounded-2xl bg-[#f8f4ee] p-5 text-left"><p className="text-xs font-bold uppercase tracking-wider text-[#b65347]">Linked learner performance</p>{portal.data.children.length ? portal.data.children.map(child => <div key={child.id} className="mt-3"><p className="font-semibold text-[#172338]">{child.fullName} {child.surname}</p><p className="text-xs text-[#776f6b]">{child.className}</p></div>) : <p className="mt-3 text-sm text-[#776f6b]">Your account is active. Learner records will appear once linked by the school.</p>}{portal.data.performance.length ? <div className="mt-4 space-y-2"><p className="text-xs font-bold uppercase tracking-wider text-[#b65347]">Performance</p>{portal.data.performance.map(entry => <div key={entry.id} className="flex justify-between text-sm"><span>{entry.activityName}</span><strong>{entry.marks}/{entry.totalMarks} ({Math.round(entry.marks / entry.totalMarks * 100)}%)</strong></div>)}</div> : <p className="mt-4 text-sm text-[#776f6b]">No performance entries have been published yet.</p>}{<div className="mt-5 border-t border-[#eadfd3] pt-4"><p className="text-xs font-bold uppercase tracking-wider text-[#b65347]">Attendance by learner</p>{portal.data.children.length ? <div className="mt-3 space-y-4">{portal.data.children.map(child => { const childAttendance = portal.data.attendance.filter(record => record.learnerId === child.id).slice(0, 12); return <div key={`attendance-${child.id}`} className="rounded-xl bg-white/70 p-3"><p className="text-sm font-semibold text-[#172338]">{child.fullName} {child.surname}</p>{childAttendance.length ? <div className="mt-2 space-y-1.5">{childAttendance.map(record => <div key={record.id} className="flex items-center justify-between text-sm"><span>{new Date(record.attendanceDate).toLocaleDateString()}</span><strong className="capitalize">{record.status}</strong></div>)}</div> : <p className="mt-2 text-xs text-[#776f6b]">No attendance records yet.</p>}</div>})}</div> : <p className="mt-3 text-sm text-[#776f6b]">No linked learners have attendance records yet.</p>}</div>}{portal.data.updates.length ? <div className="mt-5 border-t border-[#eadfd3] pt-4"><p className="text-xs font-bold uppercase tracking-wider text-[#b65347]">Urgent school updates</p>{portal.data.updates.map(update => <div key={update.id} className="mt-3"><p className="font-semibold">{update.title}</p><p className="mt-1 text-sm leading-6 text-[#776f6b]">{update.body}</p><button type="button" className="mt-2 text-xs font-bold text-[#b65347]" onClick={() => markRead.mutate({ updateId: update.id })}>{portal.data.readIds.includes(update.id) ? "Read" : "Mark as read"}</button></div>)}</div> : null}</div>}
            </div>
