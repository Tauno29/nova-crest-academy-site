          ...
          <section className="w-full max-w-[980px] rounded-[2rem] border border-[#eee1d8] bg-white p-7 shadow-[0_18px_50px_rgba(39,27,18,.08)] sm:p-10">
            {portal.isLoading ? <p className="text-center text-sm text-[#6d625b]">Loading learner record…</p> : learner ? <>
              ...
              <div className="mt-5 rounded-2xl border border-[#eee1d8] p-5">
                ...
                {(portal.data?.attendance.length ?? 0) > 0 ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{portal.data?.attendance.slice(0, 6).map((entry) => <div key={entry.id} className="flex items-center justify-between gap-3 rounded-xl bg-[#fffaf7] px-3 py-3 text-sm"><span className="text-[#6d625b]">{new Date(entry.attendanceDate).toLocaleDateString()}</span><span className={`font-extrabold ${entry.status.toLowerCase() === "present" ? "text-[#4b805b]" : "text-[#a73b32]"}`}>{entry.status}</span></div>)}</div> : <EmptyRecordState>No attendance records have been published yet.</EmptyRecordState>}
              </div>
